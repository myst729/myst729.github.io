...
有了孤岛的 commit hash，就可以上岸了吧。没错，这就是 `git checkout`。说到这里，有的同学可能很困扰，`git checkout` 这个命令既可以检出分支，又可以检出标签，还可以检出 commit hash，加上 `-b` 参数甚至还能创建分支，感觉好乱。看晚上一篇以及本篇前面的内容，应该就比较清楚了。`git checkout` 做的事情只有一件，就是检出某个 commit 对象（你可以试试检出其他类型的哈希对象，并不会成功）。好了，上岸吧。
